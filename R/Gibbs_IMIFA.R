...
# Gibbs Sampler Function
  .gibbs_IMIFA       <- function(Q, data, iters, N, P, G, mu.zero, rho, sigma.l, learn.alpha, mu, sw, uni.type, uni.prior, col.mean,
                                ...
                                #global.shrink, omega1, omega2, hetero, eta.shape, eta.rate
  ...
  # Define & initialise variables
   ...
   #  eta            <- .sim_het_p(N=N, Q=Qmax, eta.sig=eta.sig)
  ...
  # Iterate
            ...
            for(t in Ts[!nn0][Qemp      != Qmax]) {
              ...
              if(Qt   > Qmax)  {
               ...
               #eta.sig[[t]]  <- eta.sig[[t]][Qmaxseq]
                ...
                #eta.sig[[t]] <- cbind(eta.sig[[t]], .rgamma0(Qt, shape=eta.shape, rate=eta.rate))
    ...
    # Label Switching
      ...
      if(storage)    {
       ...
       #if(hetero)       {         ll.store[new.it]    <-   if(G > 1) sum(rowLogSumExps(log.probs, useNames=FALSE)) else sum(dmvn(X=data, mu=mu[,nn.ind], sigma=tcrossprod(lmat[[nn.ind]] %*% diag(eta.sig[seq_len(Qs[nn.ind])], Qs[nn.ind]), lmat[nn.ind]) + diag(psi.store[,nn.ind,new.it]), log=TRUE))
       #                 } else    ll.store[new.it]    <-   if(G > 1) sum(rowLogSumExps(log.probs, useNames=FALSE)) else sum(dmvn(X=data, mu=mu[,nn.ind], sigma=tcrossprod(lmat[[nn.ind]]) + diag(psi.store[,nn.ind,new.it]), log=TRUE))
