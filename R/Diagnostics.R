...
get_IMIFA_results.IMIFA        <- function(sims = NULL, burnin = 0L, thinning = 1L, G = NULL, Q = NULL, criterion = c("bicm", "aicm", "dic", "bic.mcmc", "aic.mcmc"), G.meth = c("mode", "median"),
 ...
 #gshrink        <- attr(sims, "G.Shrink") # vignette file will need this attribute     / need posterior mean ?
 #hetero         <- attr(sim,  "Hetero")   # vignette file will NOT need this attribute / need posterior mean ?
  ...
  # Control for supplied values of G &/or Q
    ...
    if(inf.Q) {
     ...
     #if(gshrink) {
     #  SIGMAS   <- sims[[G.ind]][[Q.ind]]$SIGMA[tmp.store]
     #}
    }
...
# Loop over g in G to extract other results
  ...
  if(sw["l.sw"]  <- tmpsw["l.sw"] && !all(Q == 0)) {
   ...
   #if(gshrink)     attr(loads, "SIGMA")    <- GQ.res$post.SIGMA
    class(loads) <- "listof"
  }
  if(sw["psi.sw"]) {
...
# Calculate estimated covariance matrices & compute error metrics
 ...
 #attr(result, "G.Shrink")     <- gshrink
 #attr(result, "Hetero")       <- hetero
